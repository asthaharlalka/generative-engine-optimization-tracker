...
# ── Helper: call OpenAI ───────────────────────────────────────────────────────
def query_openai(question: str, client: OpenAI, max_tokens: int = 500) -> str:
    try:
...
def query_perplexity(question: str, api_key: str, max_tokens: int = 500) -> str:
    try:
        headers = {
        ...
        if "choices" not in data:
            return f"ERROR: Perplexity — {data.get('error', {}).get('message', str(data))}"
...
def query_claude(question: str, api_key: str, max_tokens: int = 500) -> str:
    try:
        headers = {
            "x-api-key": api_key,
        ...
        if "content" not in data:
            return f"ERROR: Claude — {data.get('error', {}).get('message', str(data))}"
...
def query_gemini(question: str, api_key: str, max_tokens: int = 600, json_mode: bool = False) -> str:
    try:
        url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={api_key}"
        gen_config = {"maxOutputTokens": max_tokens, "temperature": 0.2 if json_mode else 0.3}
        if json_mode:
            gen_config["responseMimeType"] = "application/json"
        payload = {
            "systemInstruction": {"parts": [{"text": "You output ONLY what is asked. No prose, no commentary, no headings."}]},
        ...
        if not cands:
            return f"ERROR: Gemini — no candidates returned: {data.get('error', {}).get('message', str(data))}"
        content = cands[0].get("content", {})
        parts = content.get("parts") or []
        text_parts = [p.get("text", "") for p in parts if isinstance(p, dict)]
        text = "\n".join(t for t in text_parts if t).strip()
        if not text:
            return f"ERROR: Gemini — empty response: {str(data)[:200]}"
        return text
    except Exception as e:
        return f"ERROR: {str(e)}"
...
def query_grok(question: str, api_key: str, max_tokens: int = 500) -> str:
    try:
        headers = {
            "Authorization": f"Bearer {api_key}",
            "Content-Type": "application/json"
        ...
        if not isinstance(data, dict):
            return f"ERROR: Grok — unexpected response: {str(data)[:200]}"
        if "choices" not in data:
            error = data.get("error", {})
            msg = error.get("message", str(data)) if isinstance(error, dict) else str(error)
            return f"ERROR: Grok — {msg}"
        return data["choices"][0]["message"]["content"]
    except Exception as e:
        return f"ERROR: {str(e)}"
...
with btn_col2:
    if st.button("⏹  Stop Scan", use_container_width=True):
        ...
        st.warning("Stop requested — will halt after the current question finishes.")
...
if run:
    ...
    with st.spinner(f"Generating 10 buyer questions for '{category}'..."):
        ...
        # Standard numbered list prompt (for GPT, Claude, Grok, Perplexity)
        gen_q = f"""You are building a brand visibility scanner. Generate exactly 10 questions that measure how well AI recommends brands in a category.
...
2. NO BRAND NAMES IN THE QUESTION: Keep it open — any brand could win.
3. NO HEAD-TO-HEAD: No "X vs Y" or "which is better, X or Y" formats.
4. NO GENERIC EDUCATION: No "how do I choose..." questions. Must ask for brand recommendations.
5. COVER DIFFERENT BUYING CONTEXTS: Use cases, buyer types, price points, expertise levels.
6. MATCH CATEGORY TYPE: Consumer → shopper questions. B2B software → business buyer questions.

Return ONLY the 10 questions as a numbered list (1. 2. 3. ...). Keep each question under 15 words. No intro, no explanation, nothing else."""

        # JSON prompt for Gemini — more reliable than regex parsing
        gen_q_json = f"""Generate exactly 10 brand-intent questions for the category: "{category}".
Brands being tracked: {brand_list_str}

Rules:
- Each question must naturally require recommending one or more brands in its answer
- Do NOT name any of the tracked brands in the question itself
- No head-to-head comparisons (no "X vs Y")
- No generic "how to choose" education questions
- Cover different use cases, buyer types, price points
- Keep each question under 15 words

Return ONLY a JSON array of 10 strings. Example format:
["Question one?", "Question two?", ...]"""

        if "gpt" in llm_map:
            raw = query_openai(gen_q, llm_map["gpt"]["args"][0], max_tokens=2000)
        elif "claude" in llm_map:
            raw = query_claude(gen_q, claude_key, max_tokens=2000)
        elif "gemini" in llm_map:
            raw = query_gemini(gen_q_json, gemini_key, max_tokens=1000, json_mode=True)
        elif "grok" in llm_map:
            raw = query_grok(gen_q, grok_key, max_tokens=2000)
        else:
            raw = query_perplexity(gen_q, perp_key, max_tokens=2000)
        ...
        def extract_json_array(text: str):
            ...
            try:
                ...
                pass
            # Try to salvage truncated JSON — find first [...] block
        ...
        # If Gemini returned truncated JSON, retry once with higher token limit
        if parsed_list is None and "gemini" in llm_map:
            raw_retry = query_gemini(gen_q_json, gemini_key, max_tokens=1500, json_mode=True)
            parsed_list = extract_json_array(raw_retry)
            if parsed_list is not None:
                raw = raw_retry  # show latest in debug

        if isinstance(parsed_list, list):
            generated_questions = [str(q).strip().strip('"') for q in parsed_list if str(q).strip()]
        else:
            # Line-by-line fallback — handles 1. / 1) / 1: / 1- and bullets
            for line in raw.strip().split("\n"):
                ...
                if match:
                    generated_questions.append(match.group(1).strip().strip('"'))
                    continue
                match2 = re.match(r'^\s*[-•]\s*(.+)', line)
                if match2:
                    generated_questions.append(match2.group(1).strip().strip('"'))
        ...
        if len(generated_questions) < 5:
            st.error(f"Only {len(generated_questions)} valid questions generated — need at least 5.")
            with st.expander("🔍 Debug — full raw API response", expanded=True):
                st.markdown(f"**Raw length:** {len(raw)} chars")
                try:
                    json.loads(raw.strip())
                except Exception as e:
                    st.markdown(f"**JSON parse failed:** `{e}`")
    ...
    for i, question in enumerate(questions):
        if st.session_state.get("stop_scan", False):
            st.session_state.stop_scan = False
            st.warning(f"Scan stopped after {i}/{len(questions)} questions. Partial results shown below.")
            break

        status.markdown(
            f'<div style="font-family: DM Mono; font-size: 0.75rem; color: #7c3aed;">'
            f'Querying question {i+1}/{len(questions)}... (click ⏹ Stop Scan above to cancel)</div>',
            unsafe_allow_html=True
        )
...
Raw JSON only. No markdown fences, no explanation."""
    ...
    with st.spinner(f"Analyzing scan data and generating playbook for {selected_brand}..."):
        if "gpt" in llm_map:
            raw_playbook = query_openai(playbook_prompt, llm_map["gpt"]["args"][0], max_tokens=2000)
        elif "claude" in llm_map:
            raw_playbook = query_claude(playbook_prompt, claude_key, max_tokens=2000)
        elif "grok" in llm_map:
            raw_playbook = query_grok(playbook_prompt, grok_key, max_tokens=2000)
        else:
            raw_playbook = query_perplexity(playbook_prompt, perp_key, max_tokens=2000)

    try:
        clean = re.sub(r"```json|```", "", raw_playbook).strip()
        # Extract JSON array even if there's text before/after it
        match = re.search(r'\[.*\]', clean, re.DOTALL)
        if match:
            clean = match.group(0)
        steps = json.loads(clean)
    except Exception:
        st.warning("Couldn't parse playbook format — showing raw response:")
        st.code(raw_playbook, language="text")
        steps = []
...
Raw JSON only. No markdown, no explanation."""
        ...
        with st.spinner("Analyzing competitor patterns..."):
            if "gpt" in llm_map:
                raw_intel = query_openai(comp_intel_prompt, llm_map["gpt"]["args"][0], max_tokens=2000)
            elif "claude" in llm_map:
                raw_intel = query_claude(comp_intel_prompt, claude_key, max_tokens=2000)
            elif "grok" in llm_map:
                raw_intel = query_grok(comp_intel_prompt, grok_key, max_tokens=2000)
            else:
                raw_intel = query_perplexity(comp_intel_prompt, perp_key, max_tokens=2000)

        try:
            clean_intel = re.sub(r"```json|```", "", raw_intel).strip()
            match_intel = re.search(r'\[.*\]', clean_intel, re.DOTALL)
            if match_intel:
                clean_intel = match_intel.group(0)
