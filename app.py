...
<div class="hero">
    ...
    <div>
        ...
        <span class="stat-pill">10 buyer questions per scan</span>
        <span class="stat-pill">GPT-4o + Perplexity</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    category = st.text_input(
        ...
        placeholder="e.g. GTM Automation, ABM Software, CDP Platforms..."
    )

with col2:
    st.markdown('<div class="section-label">Step 2 — Your brand</div>', unsafe_allow_html=True)
    your_brand = st.text_input(
        "Your brand",
        value="",
        label_visibility="collapsed",
        placeholder="e.g. PiVector"
    )
...
brands_input = st.text_input(
    "Competitor brands",
    value="Salesforce, HubSpot, Pipedrive, Zoho CRM, Monday CRM",
    label_visibility="collapsed",
...
custom_questions_raw = st.text_area(
    ...
    value="",
    label_visibility="collapsed",
    placeholder="Which coffee brand do professional baristas use?\nBest coffee for someone who hates bitter taste?\nWhat coffee brand is worth the premium price?",
    height=120
)
...
if run:
    ...
    if len(brands) < 2:
        st.error("Enter at least 2 brands to compare.")
        st.stop()
...
Return ONLY the 10 questions as a numbered list. No intro, no explanation, nothing else."""
        ...
        def is_valid_question(q):
            ...
            if vs_pattern.search(q):
                return False
            # Reject if it has no brand-seeking intent
            brand_intent_signals = ["which", "what", "best", "recommend", "brand", "who makes", "top", "should i"]
            if not any(s in q_lower for s in brand_intent_signals):
                return False
            return True
        ...
        if len(generated_questions) < 5:
            st.error("Couldn't generate enough valid questions for that category. Try being more specific.")
            st.stop()
    ...
    for i, question in enumerate(questions):
        ...
        <div class="question-row">
            ...
            <span style='font-family: DM Mono; font-size: 0.7rem; color: #7c3aed;'>
                Brands mentioned: {mentioned_str}
            </span>
        </div>"""
    ...
    st.markdown("---")
    st.markdown('<div class="section-label">GEO Optimization Playbook</div>', unsafe_allow_html=True)
    st.markdown(
        '<div style="font-family: DM Mono; font-size: 0.72rem; color: #555; margin-bottom: 1.25rem;">'
        'Select a brand to see exactly what they should do to improve their AI visibility score.'
        '</div>',
        unsafe_allow_html=True
    )

    default_brand_idx = 0
    if your_brand.strip() and your_brand.strip() in df["Brand"].values:
        default_brand_idx = list(df["Brand"].values).index(your_brand.strip())

    selected_brand = st.selectbox(
        "Choose brand to optimize",
        options=list(df["Brand"].values),
    ...
    # Figure out which questions they were missing from
    missed_questions = []
    for r in results:
    ...
    if score == 0:
        tier = "invisible"
        tier_label = "🔴 Invisible (Score: 0)"
        tier_color = "#ef4444"
    elif score <= 30:
        tier = "weak"
        tier_label = f"🟡 Weak Presence (Score: {score})"
        tier_color = "#f59e0b"
    elif score <= 60:
        tier = "emerging"
        tier_label = f"🟠 Emerging (Score: {score})"
        tier_color = "#f97316"
    else:
        tier = "visible"
        tier_label = f"🟢 Visible (Score: {score})"
        tier_color = "#22c55e"
    ...
    <div style="background: #0d0d18; border: 1px solid #1e1e2e; border-left: 3px solid {tier_color};
        ...
        <div style="font-family: Syne; font-size: 0.9rem; color: #aaa; line-height: 1.6;">
            ...
            Missed <strong style="color:#e8e8f0">{missed_q}</strong> opportunities.
            {"This is the category leader — focus on defending and widening the gap." if gap_vs_winner == 0
             else f"Gap vs AI leader: <strong style='color:#ef4444'>{gap_vs_winner} points</strong>."}
        </div>
    </div>
    """, unsafe_allow_html=True)

    # ── Missed questions callout ───────────────────────────────────────────────
    if missed_questions:
        with st.expander(f"📋 {len(missed_questions)} buyer questions {selected_brand} didn't appear in"):
            ...
            st.markdown(
                "<div style='font-family: DM Mono; font-size: 0.68rem; color: #444; margin-top: 0.75rem;'>"
                "These are the exact questions to write content around. Each one is a content brief."
                "</div>",
                unsafe_allow_html=True
            )
    ...
    PLAYBOOK = {
        "invisible": [
            {
                "priority": "🚨 URGENT",
                "title": "Create a dedicated 'Best [Category] Software' comparison page",
                "why": "LLMs pull from pages that directly answer category comparison questions. If you don't have a page titled something like 'Best CRM Software in 2025' that mentions your product, you won't appear.",
                "how": "Write a 1,500–2,000 word page on your own site comparing your product to the top 4–5 competitors. Be honest — LLMs distrust one-sided content. Include a clear positioning statement for each. Publish it at /compare or /alternatives.",
                "effort": "3–5 days",
                "impact": "High"
            },
            {
                "priority": "🚨 URGENT",
                "title": "Get listed on G2, Capterra, and Trustpilot — and get reviews",
                "why": "LLMs heavily weight third-party review platforms as trusted signals. Brands with 0–5 reviews are invisible. Brands with 50+ reviews with keyword-rich content appear constantly.",
                "how": "Claim your G2 and Capterra profiles today. Email your 10 happiest customers asking for a specific, detailed review (not just stars — sentences matter). Aim for 20+ reviews in 60 days.",
                "effort": "1–2 weeks",
                "impact": "High"
            },
            {
                "priority": "🚨 URGENT",
                "title": "Answer the exact questions buyers ask on Reddit and Quora",
                "why": "Reddit is a primary training source for most LLMs. If buyers ask 'best CRM for startups' on r/startups and your brand never appears in the thread, you don't exist.",
                "how": "Search Reddit for the questions in this scan. Join the conversation authentically — don't spam. Answer genuinely and mention your product where relevant. Also create a Quora answer for each question.",
                "effort": "Ongoing",
                "impact": "High"
            },
            {
                "priority": "📌 IMPORTANT",
                "title": "Publish an FAQ page that mirrors buyer question language",
                "why": "LLMs love FAQ-structured content because it directly maps to question-answer format. If your site has a page that literally asks and answers 'What is the best [category] tool for [use case]?', you will start appearing.",
                "how": f"Take the {len(missed_questions)} questions from this scan that you missed. Write a FAQ page that answers each one. Be specific, include real comparisons, and link to your product pages.",
                "effort": "1 week",
                "impact": "Medium-High"
            },
            {
                "priority": "📌 IMPORTANT",
                "title": "Get press mentions in industry publications",
                "why": "LLMs are trained on high-authority web content. A mention in TechCrunch, G2 news, Product Hunt, or a niche industry blog signals credibility to the model.",
                "how": "Start with Product Hunt launch (free). Then pitch a data-driven story to 2–3 industry blogs. Even a 'Top 10 [Category] Tools' roundup mention on a DA 40+ site moves the needle.",
                "effort": "2–4 weeks",
                "impact": "Medium"
            }
        ],
        "weak": [
            {
                "priority": "🔥 HIGH",
                "title": "Deepen your comparison content — go specific on use cases",
                "why": "You're appearing in some questions but not others. That usually means you have broad category coverage but are missing use-case-specific content.",
                "how": f"Look at the {len(missed_questions)} questions you missed. They likely follow a pattern — a specific industry, company size, or feature. Write dedicated pages for each gap (e.g. 'Best CRM for SaaS companies', 'Best CRM with Slack integration').",
                "effort": "2–3 weeks",
                "impact": "High"
            },
            {
                "priority": "🔥 HIGH",
                "title": "Build a structured 'vs competitor' content library",
                "why": "When a buyer asks 'X vs Y', LLMs look for pages that explicitly address that comparison. If you have no '[Your Brand] vs [Competitor]' pages, you lose those questions entirely.",
                "how": "Create one '[Your Brand] vs [Competitor]' page for each of the brands in this scan. Keep them honest and balanced — LLMs penalize obviously biased content. Focus on who each product is actually better for.",
                "effort": "1–2 weeks per page",
                "impact": "High"
            },
            {
                "priority": "🔥 HIGH",
                "title": "Increase your review volume and recency on G2/Capterra",
                "why": "LLMs factor in review recency and volume. A brand with 200 reviews from 2021 can lose to a brand with 40 reviews from 2024.",
                "how": "Run a quarterly review ask to your active customer base. Set up an automated NPS follow-up that routes happy customers (9–10) to your G2 page. Target 5+ new reviews per month.",
                "effort": "Ongoing",
                "impact": "Medium-High"
            },
            {
                "priority": "📌 MEDIUM",
                "title": "Create customer success stories optimized for AI discovery",
                "why": "Case studies that mention specific problems, industries, and outcomes get cited by LLMs when they answer 'best X for Y' questions.",
                "how": "Rewrite your case studies to include: the exact category terms buyers search, the company size, the specific problem solved, and quantified outcomes. Use headers like 'How [Company] chose [Your Brand] over [Competitor]'.",
                "effort": "1 week",
                "impact": "Medium"
            }
        ],
        "emerging": [
            {
                "priority": "🔥 HIGH",
                "title": "Target the specific questions you're still missing",
                "why": "You're visible but not dominant. The gap between you and the leader is about coverage — they appear in questions you're still missing.",
                "how": f"Run this scan again focused on just those {len(missed_questions)} missed questions. Diagnose whether you're missing by industry, company size, feature, or use case. Write one dedicated piece of content per gap.",
                "effort": "2–3 weeks",
                "impact": "High"
            },
            {
                "priority": "📌 MEDIUM",
                "title": "Get cited by LLM-visible sources — not just SEO-visible ones",
                "why": "Some sources are SEO-ranked but rarely appear in LLM training. Focus on sources that LLMs actually cite: Wikipedia, Reddit, Hacker News, Product Hunt, peer-reviewed industry reports.",
                "how": "Check if your brand is mentioned on Wikipedia's 'list of [category] software' page. If not, contribute. Answer questions on Hacker News 'Ask HN' threads about your category. Submit your product to relevant curated lists.",
                "effort": "Ongoing",
                "impact": "Medium-High"
            },
            {
                "priority": "📌 MEDIUM",
                "title": "Publish original research or data in your category",
                "why": "LLMs cite data. If your company publishes an annual report, benchmark study, or industry survey, other sites cite it — and LLMs pick that up.",
                "how": "Survey 100+ customers or prospects. Publish a 'State of [Category]' report. Pitch the findings to industry journalists. This creates a citation chain that builds AI visibility over 6–12 months.",
                "effort": "4–6 weeks",
        ...
        "visible": [
            {
                "priority": "🛡️ DEFEND",
                "title": "Monitor for category drift — your questions may change",
                "why": "LLM training data updates. A brand that's visible today may not be in 6 months if the category language shifts or a competitor builds more content.",
                "how": "Run this scan monthly with the same questions. Track score over time. Set a floor — if your score drops below X, it's a content alarm.",
                "effort": "1 hour/month",
                "impact": "Defensive"
            },
            {
                "priority": "🛡️ DEFEND",
                "title": "Expand into adjacent category questions",
                "why": "You own your core category. The next growth move is appearing in adjacent queries — buyer questions that overlap with your category but aren't the main term.",
                "how": "Run this scan on adjacent categories (e.g. if you're CRM, also run it on 'Sales Engagement', 'Revenue Operations', 'Pipeline Management'). Find where you already appear and double down.",
                "effort": "Ongoing",
                "impact": "Growth"
            },
            {
                "priority": "📌 MEDIUM",
                "title": "Turn your AI visibility into a marketing asset",
                "why": "Being AI-visible is a differentiator right now. Most companies don't know their score — you do.",
                "how": "Add a page to your site or your sales deck: 'When buyers ask AI about [category], [Your Brand] appears in X out of 10 responses.' Use this in competitive conversations.",
                "effort": "1 day",
                "impact": "Marketing"
            }
        ]
    ...
    for step in steps:
        impact_color = "#22c55e" if step["impact"] == "High" else "#f59e0b" if "Medium" in step["impact"] else "#888"
        st.markdown(f"""
        <div style="background: #0a0a0f; border: 1px solid #1e1e2e; border-radius: 8px;
            ...
            <div style="font-family: DM Mono; font-size: 0.75rem; color: #666; line-height: 1.6; margin-bottom: 0.6rem;">
                <strong style="color: #555;">Why this works:</strong> {step['why']}
            </div>
    ...
    <div style="text-align: center; padding: 4rem 2rem; color: #333;">
        ...
        <div style="font-family: DM Mono; font-size: 0.8rem; letter-spacing: 0.1em;
                    text-transform: uppercase; color: #444;">
            Add your API keys in the sidebar, choose a category, enter competitor brands, then hit Run.
        </div>
        <div style="margin-top: 1.5rem; font-size: 0.85rem; color: #333; max-width: 400px; margin-left: auto; margin-right: auto;">
            The scan fires 10 real buyer questions at ChatGPT and Perplexity and counts how many times each brand appears.
        </div>
    </div>
    """, unsafe_allow_html=True)
