...
Return ONLY the 10 questions as a numbered list. No intro, no explanation, nothing else."""
    ...
    # ── AI-Generated Optimization Playbook ───────────────────────────────────
    # Build what LLMs actually said about this brand vs what they said about the winner
    winner_responses_sample = []
    brand_responses_sample = []
    for r in results[:5]:  # first 5 questions for context brevity
        winner_text = " | ".join(
            r.get(f"{llm}_response", "")[:200]
            for llm in llm_keys
            if winner["Brand"].lower() in r.get(f"{llm}_response", "").lower()
        )
        brand_text = " | ".join(
            r.get(f"{llm}_response", "")[:200]
            for llm in llm_keys
            if selected_brand.lower() in r.get(f"{llm}_response", "").lower()
        )
        if winner_text:
            winner_responses_sample.append(f"Q: {r['question']}\nHow AI described {winner['Brand']}: {winner_text[:300]}")
        if brand_text:
            brand_responses_sample.append(f"Q: {r['question']}\nHow AI described {selected_brand}: {brand_text[:300]}")

    playbook_prompt = f"""You are a GEO (Generative Engine Optimization) strategist. A brand has just run a scan measuring how often AI recommends them vs competitors. Generate a specific, actionable optimization playbook based on the real scan data below.

SCAN DATA:
- Category: {category}
- Brand being analyzed: {selected_brand}
- Visibility Score: {score}/100
- Appeared in: {appeared}/{total_q} buyer questions
- Missed {missed_q} questions
- Gap vs category leader ({winner['Brand']}): {gap_vs_winner} points
- Active LLMs in scan: {", ".join(llm_labels.get(k, k) for k in llm_keys)}

QUESTIONS {selected_brand.upper()} MISSED:
{chr(10).join(f"- {q}" for q in missed_questions) if missed_questions else "None — brand appeared in all questions"}

HOW AI DESCRIBED THE CATEGORY LEADER ({winner['Brand'].upper()}) in responses:
{chr(10).join(winner_responses_sample) if winner_responses_sample else "Not captured"}

HOW AI DESCRIBED {selected_brand.upper()} when it did appear:
{chr(10).join(brand_responses_sample) if brand_responses_sample else "Brand did not appear in any responses"}

Generate exactly 4 specific optimization recommendations. Each must be directly informed by the scan data above — reference the actual missed questions, the actual category, and the actual gap. Do NOT give generic advice.

Return ONLY a JSON array with exactly 4 objects. Each object must have these exact keys:
- "priority": one of "🚨 URGENT", "🔥 HIGH", "📌 MEDIUM", "🛡️ DEFEND"
- "title": short action title (max 10 words)
- "why": why this specific brand has this specific gap (2-3 sentences, reference actual missed questions or category context)
- "how": concrete step-by-step actions (3-4 sentences, specific to this category and brand situation)
- "effort": time estimate like "3-5 days" or "2-3 weeks"
- "impact": one of "High", "Medium-High", "Medium", "Defensive", "Growth"

Return raw JSON only. No markdown, no backticks, no explanation."""

    with st.spinner(f"Generating personalized GEO playbook for {selected_brand}..."):
        if "gpt" in llm_map:
    ...
    # Parse the JSON
    try:
        clean = raw_playbook.strip().lstrip("```json").lstrip("```").rstrip("```").strip()
    ...
    for step in steps:
        ...
        <div style="background: #0a0a0f; border: 1px solid #1e1e2e; border-radius: 8px;
            ...
            <div style="font-family: DM Mono; font-size: 0.75rem; color: #666; line-height: 1.6; margin-bottom: 0.6rem;">
                <strong style="color: #555;">Why this works:</strong> {step.get('why', '')}
            </div>
