...
["Question one?", "Question two?", ...]"""
        ...
        # Parse — try JSON first (Gemini returns JSON array), fall back to line parsing
        generated_questions = []
        try:
            parsed = json.loads(raw.strip())
            if isinstance(parsed, list):
                generated_questions = [str(q).strip().strip('"') for q in parsed if str(q).strip()]
        except Exception:
            # Line-by-line parser — handles 1. / 1) / 1: / 1 - and bullet formats
            for line in raw.strip().split("\n"):
                line = re.sub(r'\*+', '', line).strip()
                if not line:
                    continue
                # Numbered formats: 1. / 1) / 1: / 1 -
