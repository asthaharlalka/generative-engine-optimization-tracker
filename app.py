...
def query_gemini(question: str, api_key: str, max_tokens: int = 500) -> str:
    try:
...
Return ONLY the 10 questions as a numbered list. Keep each question under 15 words. No intro, no explanation, nothing else."""
        ...
        elif "perplexity" in llm_map:
            raw = query_perplexity(gen_q, perp_key, max_tokens=2000)
        else:
            # Gemini is last resort for question gen — slow but works
            raw = query_gemini(gen_q, gemini_key, max_tokens=800)

        # Robust parser — handles plain "1.", bold "**1.**", and "1)" formats
        generated_questions = []
        for line in raw.strip().split("\n"):
            line = re.sub(r'\*+', '', line).strip()  # strip bold markdown
            if not line:
                continue
...
Raw JSON only. No markdown fences, no explanation."""
    ...
    with st.spinner(f"Analyzing scan data and generating playbook for {selected_brand}..."):
        if "gpt" in llm_map:
            ...
            raw_playbook = query_gemini(playbook_prompt, gemini_key, max_tokens=2000)
        elif "grok" in llm_map:
...
Raw JSON only. No markdown, no explanation."""
        ...
        with st.spinner("Analyzing competitor patterns..."):
            if "gpt" in llm_map:
                ...
                raw_intel = query_gemini(comp_intel_prompt, gemini_key, max_tokens=2000)
            elif "grok" in llm_map:
