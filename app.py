...
for key in ["openai_key", "perp_key", "claude_key", "gemini_key", "grok_key", "scanning"]:
    if key not in st.session_state:
        st.session_state[key] = "" if key != "scanning" else False
...
# ── Run / Stop button ─────────────────────────────────────────────────────────
run_col, stop_col, _ = st.columns([1, 1, 1])
with run_col:
    run = st.button("🔍  Run GEO Scan", use_container_width=True, disabled=st.session_state.scanning)
with stop_col:
    if st.session_state.scanning:
        if st.button("⏹  Stop Scan", use_container_width=True, type="secondary"):
            st.session_state.scanning = False
            st.warning("Scan stopped. Results above may be partial.")
            st.stop()
...
Return ONLY the 10 questions as a numbered list. No intro, no explanation, nothing else."""
        ...
        for line in raw.strip().split("\n"):
            ...
            if not line:
                continue
            # Strip markdown bold/italic formatting Gemini sometimes adds
    ...
    for i, question in enumerate(questions):
        ...
        if not st.session_state.get("scanning", True):
            st.warning(f"Scan stopped after {i}/{len(questions)} questions. Results below are partial.")
            break
