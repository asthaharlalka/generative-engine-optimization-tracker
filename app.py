...
with btn_col2:
    if st.button("⏹  Stop Scan", use_container_width=True):
        ...
        st.warning("Stop requested — halts after current question finishes.")
...
Return ONLY the 10 questions as a numbered list. No intro, no explanation, nothing else."""
        ...
        if "gpt" in llm_map:
            ...
            raw = query_gemini(gen_q, gemini_key, max_tokens=2048)
        else:
    ...
    for i, question in enumerate(questions):
        if st.session_state.get("stop_scan", False):
            st.session_state.stop_scan = False
            st.warning(f"Stopped after {i}/{len(questions)} questions. Partial results below.")
            break

        status.markdown(
            f'<div style="font-family: DM Mono; font-size: 0.75rem; color: #7c3aed;">'
            f'Querying question {i+1}/{len(questions)}... (hit ⏹ Stop Scan above to cancel)</div>',
            unsafe_allow_html=True
        )
